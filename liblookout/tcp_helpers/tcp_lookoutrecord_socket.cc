...
void TCPLookOutRecordSocket<AdaptT>::_tcp_loop(const function<bool()> &condition) {
...
template <typename AdaptT>
TCPLookOutRecordSocket<AdaptT>::TCPLookOutRecordSocket(pair<FileDescriptor, FileDescriptor> data_socket_pair,
...
template <typename AdaptT>
void TCPLookOutRecordSocket<AdaptT>::_initialize_TCP(const TCPConfig &config) {
    _tcp.emplace(config);
...
//! \param[in] dgramfd is the FileDescriptor for reading and writing datagrams
template <typename AdaptT>
TCPLookOutRecordSocket<AdaptT>::TCPLookOutRecordSocket(FileDescriptor &&dgramfd)
    : TCPLookOutRecordSocket(socket_pair_helper(SOCK_STREAM), move(dgramfd)) {}

template <typename AdaptT>
TCPLookOutRecordSocket<AdaptT>::~TCPLookOutRecordSocket() {
    try {
        if (_tcp_thread.joinable()) {
            cerr << "Warning: unclean shutdown of TCPLookOutRecordSocket\n";
            // force the other side to exit
            _abort.store(true);
            _tcp_thread.join();
        }
    } catch (const exception &e) {
        cerr << "Exception destructing TCPLookOutRecordSocket: " << e.what() << endl;
    }
}

template <typename AdaptT>
void TCPLookOutRecordSocket<AdaptT>::wait_until_closed() {
    shutdown(SHUT_RDWR);
    if (_tcp_thread.joinable()) {
...
template <typename AdaptT>
void TCPLookOutRecordSocket<AdaptT>::connect(const TCPConfig &c_tcp, const FdAdapterConfig &c_ad) {
    if (_tcp) {
    ...
    _tcp_thread = thread(&TCPLookOutRecordSocket::_tcp_main, this);
}

//! \param[in] c_tcp is the TCPConfig for the TCPConnection
//! \param[in] c_ad is the FdAdapterConfig for the FdAdapter
template <typename AdaptT>
void TCPLookOutRecordSocket<AdaptT>::listen_and_accept(const TCPConfig &c_tcp, const FdAdapterConfig &c_ad) {
    if (_tcp) {
...
template <typename AdaptT>
void TCPLookOutRecordSocket<AdaptT>::_tcp_main() {
    try {
...
//! Specialization of TCPLookOutRecordSocket for TCPOverUDPSocketAdapter
template class TCPLookOutRecordSocket<TCPOverUDPSocketAdapter>;

//! Specialization of TCPLookOutRecordSocket for TCPOverIPv4OverTunFdAdapter
template class TCPLookOutRecordSocket<TCPOverIPv4OverTunFdAdapter>;

//! Specialization of TCPLookOutRecordSocket for LossyTCPOverUDPSocketAdapter
template class TCPLookOutRecordSocket<LossyTCPOverUDPSocketAdapter>;

//! Specialization of TCPLookOutRecordSocket for LossyTCPOverIPv4OverTunFdAdapter
template class TCPLookOutRecordSocket<LossyTCPOverIPv4OverTunFdAdapter>;

LXTCPSocket::LXTCPSocket() : TCPOverIPv4LookOutRecordSocket(TunFD("tun144")) {}

void LXTCPSocket::connect(const Address &address) {
    ...
    TCPOverIPv4LookOutRecordSocket::connect(tcp_config, multiplexer_config);
}
